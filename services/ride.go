...
func AcceptRide(rideID string, driverID string) error {
	return database.Tx(func(tx *sqlx.Tx) error {
		ride, err := repository.GetRideByIDForUpdate(tx, rideID)
		if err != nil {
		...
		if models.RideStatus(ride.Status) != models.RideRequested {
			return error_custom.ErrRideAlreadyAccepted
		}
		...
		if err := repository.AcceptRide(tx, rideID, driverID, time.Now(), models.RideAccepted); err != nil {
			return err
		}

		if err := repository.SetDriverAvailable(tx, driverID, false); err != nil {
			return err
		}
...
func VerifyRideOTP(rideID string, driverID string, otp string) error {
	...
	attempts, err := repository.GetOTPAttempts(rideID)
	if err != nil {
		return err
	}
	if attempts >= maxOTPAttempts {
		return error_custom.ErrTooManyOTPAttempts
	}

	err = repository.VerifyRideOTP(rideID, otp)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			if _, incErr := repository.IncrementOTPAttempts(rideID); incErr != nil {
				logrus.Errorf("failed to increment otp attempts for ride %s: %v", rideID, incErr)
			}
	...
	if err := repository.ResetOTPAttempts(rideID); err != nil {
		logrus.Errorf("failed to reset otp attempts for ride %s: %v", rideID, err)
	}
